...
Run `dspec --help` (or any subcommand with `--help`) for the full flag list. Headline subcommands:

| Subcommand | What it does |
|---|---|
| `dspec code [dir]` | Code-mode TUI — file edits, plan mode, edit-gate, project-scoped sessions |
| `dspec chat` | Chat-only TUI — no filesystem access, no code mode |
| `dspec run <task>` | Headless run — read prompt, execute, exit (CI-friendly) |
| `dspec setup` | Interactive first-run config (API key, language, theme) |
| `dspec sessions [name]` | List or open a saved session |
| `dspec prune-sessions` | Drop sessions older than `--days N` |
| `dspec replay <transcript>` | Re-render a JSONL transcript without calling the model |
| `dspec diff <a> <b>` | Compare two transcripts (cost / cache / tokens) |
| `dspec events <name>` | Tail the event log for a session |
| `dspec stats [transcript]` | One-shot cost / cache breakdown |
| `dspec doctor` | Health check — API reach, config, hooks, project |
| `dspec commit` | `git add -A && git commit` with an LLM-written message |
| `dspec mcp <list\|search\|install\|inspect\|browse>` | MCP server management |
| `dspec index` | Build the local semantic index (Ollama or OpenAI-compatible embeddings) |
| `dspec version` / `dspec update` | Version info + upgrade hint |
...
| `--no-config` | Ignore `~/.dspec/config.json` for this run |
...
| `/memory [list\|show\|forget\|clear]` | Manage pinned memory (DSPEC.md + `~/.dspec/memory`) |
...
| Command | What it does |
|---|---|
| `/init [force]` | Scan project, synthesize a baseline `DSPEC.md` |
...
DeepSeek-Spec sets DECSET 1007 (alternate-scroll) only — wheel events translate to ↑/↓ keypresses for the app, but native click/drag selection is left untouched. Pass `--no-mouse` to opt out entirely.
