...
<p align="center">
  <img src="docs/assets/hero-terminal.zh-CN.svg" alt="DeepSeek-Spec code 模式预览 — 助手提出 SEARCH/REPLACE 编辑，未 /apply 不落盘" width="860"/>
</p>

<br/>

> [!TIP]
> **缓存稳定不是开关，而是循环要围绕设计的不变量。** 这就是 DeepSeek-Spec 只支持 DeepSeek 的根本原因 —— 每一层都为 DeepSeek 字节稳定的前缀缓存机制调过。
...
~~~bash
cd my-project
bunx deepseek-spec code   # 首次运行粘贴 DeepSeek API Key，之后会记住
~~~

要求 Node ≥ 22。在 macOS · Linux · Windows（PowerShell · Git Bash · Windows Terminal）都跑得顺。[去拿 DeepSeek API Key →](https://platform.deepseek.com/api_keys) · 完整 flag 看 `dspec code --help`。

`bunx` 是推荐路径 —— 不用全局安装，每次都拿最新版。如果你天天用、想把 `dspec` 装到 `PATH`，跑一次 `dspec update`。

| 命令 | 何时用 |
|---|---|
| `dspec code [dir]` | 编码 agent。**先用这个。** |
| `dspec chat` | 纯聊天 —— 不挂文件系统 / shell 工具。 |
| `dspec run "task"` | 一次性，结果流到 stdout。适合 shell 管道。 |
| `dspec doctor` | 体检：Node 版本、API Key、MCP 接线。 |
| `dspec update` | 升级 DeepSeek-Spec 本身。 |

其他子命令（`replay` · `diff` · `events` · `stats` · `index` · `mcp` · `prune-sessions`）在 `dspec --help` 和 [CLI 参考](https://esengine.github.io/DeepSeek-Reasonix/#cli)。

<details>
<summary><strong>切换工作区 · chat vs. code · 写第一个 Skill</strong></summary>

**切换工作区。** DeepSeek-Spec 把文件系统工具作用域绑定在启动目录，传 `--dir` 可以指别处。中途切换是有意不支持的（消息日志和 memory 路径会和旧根目录混在一起）—— 退出再启动。
...
/skill new my-skill --global     # ~/.dspec/skills，跨项目共用
~~~
...
一个全局 JSON 文件 `~/.dspec/config.json`，加上项目级 `<project>/.dspec/` 下的覆盖。完整的双语参考 —— 每一个 key、每一条斜杠命令、skills / memory / hooks 在磁盘上的形状 —— 都在这里：
...
| [语义索引](https://esengine.github.io/DeepSeek-Reasonix/configuration.html?lang=zh#index) | `dspec index` —— 本地 Ollama，或任何 OpenAI 兼容的 embedding 接口。 |
...
<p align="center">
  <img src="docs/assets/feature-grid.zh-CN.svg" alt="DeepSeek-Spec 能力一览 — cell-diff 渲染器、MCP、计划模式、权限、仪表盘、持久化会话、Hooks/Skills/Memory、语义检索、自动 checkpoint、/effort 旋钮、transcript 重放、事件日志" width="880"/>
</p>
...
> [!NOTE]
> DeepSeek-Spec 是开源、社区共建的项目。文末"致谢"那面贡献者墙上的每一个头像，都对应一次真实合并的 PR。

给新手准备的入门 issue —— 每个都带背景说明、代码定位、验收标准、提示 —— 全部挂在 [`good first issue`](https://github.com/esengine/DeepSeek-Reasonix/labels/good%20first%20issue) 标签下。挑任意一个还没人认领的就行。

**正在征集意见的 Discussions：**

- [#20 · CLI / TUI 设计](https://github.com/esengine/DeepSeek-Reasonix/discussions/20) —— 哪里坏了、哪里少东西、哪里你会怎么改？
- [#21 · Dashboard 设计](https://github.com/esengine/DeepSeek-Reasonix/discussions/21) —— 对着[设计稿](https://esengine.github.io/DeepSeek-Reasonix/design/agent-dashboard.html)拍砖
- [#22 · 未来功能愿望单](https://github.com/esengine/DeepSeek-Reasonix/discussions/22) —— 你希望 DeepSeek-Spec 长出什么功能？

**正在使用 DeepSeek-Spec，愿意让更多人了解它？** 欢迎将相关博客、文章、截图、演讲或视频发布到 [**Show and tell**](https://github.com/esengine/DeepSeek-Reasonix/discussions/categories/show-and-tell)。项目没有营销预算，新用户主要通过社区口碑找到这里。持续参与传播的用户将获得下方这枚徽章，颁发后会展示在贡献者墙旁：

<p align="center">
  <a href="https://github.com/esengine/DeepSeek-Reasonix/discussions/categories/show-and-tell">
    <img src="https://img.shields.io/badge/DEEPSEEK_SPEC-📣%20ADVOCATE-c4b5fd?style=for-the-badge&labelColor=0d1117" alt="DeepSeek-Spec Advocate 徽章 —— 授予持续参与传播的用户"/>
  </a>
</p>
...
> [!IMPORTANT]
> DeepSeek-Spec 是有立场的。有些事它故意 *不做* —— 列在这里方便你为自己的工作挑对工具。
...
- **完全离线 / 永远免费。** DeepSeek-Spec 需要付费的 DeepSeek API Key。要离线 / 零成本，看 Aider + Ollama 或 [Continue](https://continue.dev)。
...
如果 DeepSeek-Spec 帮你省了时间或 token，欢迎请杯咖啡。捐助不会换来 feature 优先级，也不会影响 issue 的处理顺序——就是「谢谢」。
...
下面这些朋友的工作塑造了 DeepSeek-Spec 今天的样子 —— 综合 commit 数和代码量两个维度。
