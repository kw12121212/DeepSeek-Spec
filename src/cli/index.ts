...
/** Resolution order: flag → DSPEC_DASHBOARD_HOST env → config.dashboard.host → undefined (server defaults to 127.0.0.1). */
function resolveDashboardHost(
  ...
  const fromEnv = process.env.DSPEC_DASHBOARD_HOST?.trim();
...
/** Resolution order: DSPEC_DASHBOARD_TOKEN env → config.dashboard.token (minted + persisted on first call so the URL survives CLI restarts). Min 16 chars; shorter env overrides are dropped with a warning. */
function resolveDashboardToken(noConfig: boolean): string | undefined {
  const fromEnv = process.env.DSPEC_DASHBOARD_TOKEN?.trim();
  if (fromEnv) {
