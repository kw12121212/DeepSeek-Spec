...
export interface RunOptions {
  ...
  system: string;
  budgetUsd?: number;
  /** JSONL transcript path — lets `dspec replay` / `diff` audit this run. */
  transcript?: string;
...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  if (normalizedSpecs.length > 0) {
    ...
    for (const spec of normalizedSpecs) {
      ...
      try {
        ...
        // Non-fatal — skip and continue, same as `dspec chat`. A
  ...
  registry.register("openai", () => new OpenAICustomClient(), loadOpenaiCustomModels());
  ...
  try {
    for await (const ev of loop.step(opts.task)) {
      ...
      if (ev.role === "assistant_final" && ev.stats?.usage) {
        // `dspec run` is often used in CI / scripting — we want
        // those turns to show up in `dspec stats` too so the
  ...
  if (opts.transcript) {
    ...
    process.stdout.write(`  → npx deepseek-spec replay ${opts.transcript}\n`);
  }
