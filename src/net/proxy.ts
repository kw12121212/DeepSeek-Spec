...
// `DSPEC_PROXY_DEEPSEEK_DIRECT=0` (env) for corporate firewalls that block
...
export interface InstallProxyOptions {
  ...
  url?: string;
  /** Additional NO_PROXY patterns layered on top of defaults + env. Sourced from `cfg.proxy.noProxy` / `DSPEC_NO_PROXY`. */
...
/** Env `DSPEC_PROXY_DEEPSEEK_DIRECT` (1/0/true/false/yes/no/on/off) overrides config when set; defaults true. Per issue #1497, corporate firewalls need the env knob since they often can't edit `~/.dspec/config.json` ergonomically. */
export function resolveBypassDeepSeekDirect(
  env: NodeJS.ProcessEnv,
  configValue: boolean | undefined,
): boolean {
  const raw = env.DSPEC_PROXY_DEEPSEEK_DIRECT;
...
/** Merge default + env + DSPEC_NO_PROXY + opts.extraNoProxy into one resolved view. Same composition as installProxyIfConfigured so /doctor can show what's actually applied. */
...
export function installProxyIfConfigured(
  ...
  // Default whitelist always applies; env NO_PROXY, DSPEC_NO_PROXY, and
