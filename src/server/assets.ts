...
export function renderIndexHtml(token: string, mode: "standalone" | "attached"): string {
  ...
  // template has __DSPEC_TOKEN__ in three places (meta + css href +
  ...
  // 401 on every asset fetch. Same trap for __DSPEC_MODE__ if it
  // ever appears more than once.
  return tpl.replaceAll("__DSPEC_TOKEN__", safeToken).replaceAll("__DSPEC_MODE__", mode);
}
