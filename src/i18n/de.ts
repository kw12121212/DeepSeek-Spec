...
export const de: TranslationSchema = {
  ...
  cli: {
    ...
    version: "DeepSeek-Spec-Version ausgeben.",
    update: "Nach einer neueren DeepSeek-Spec-Version suchen und installieren.",
  },
  stats: {
    ...EN.stats,
    usageHint: "Führe `dspec chat`, `dspec code` oder `dspec run <task>` aus – jeden Turn",
    usageDetail: "Hängt eine Zeile an das Log an; `dspec stats` fasst sie zusammen.",
  },
  run: {
    ...EN.run,
    missingApiKey:
      "DEEPSEEK_API_KEY ist nicht gesetzt und stdin ist kein TTY (Nachfrage nicht möglich).\n" +
      "Setze die Umgebungsvariable oder starte einmal interaktiv `dspec chat`, um einen Schlüssel zu speichern.\n",
  },
  sessions: {
    ...EN.sessions,
    emptyHint:
      "Noch keine gespeicherten Sitzungen – starte `dspec chat` (Sitzungen werden automatisch gespeichert, außer mit --no-session).",
    listHeader: "Gespeicherte Sitzungen (~/.dspec/sessions/):",
    inspectHint: "Ansehen:       dspec sessions <name>",
    resumeHint: "Fortsetzen:    dspec chat --session <name>",
  ...
  ui: {
    ...
    welcome: "Starte jederzeit `dspec`, um zu chatten – deine Einstellungen bleiben gespeichert.",
  ...
  code: {
    ...EN.code,
    workspaceConflict:
      "⚠ Arbeitsbereich enthält Dateien einer anderen Agent-Plattform ({platforms}). DeepSeek-Spec Code kann sie als Projektinhalt lesen; starte mit --dir <dein-projekt> neu, falls das nicht gewünscht ist.\n",
  ...
  slash: {
    ...
    setup: { ...EN.slash.setup, description: "Erinnert dich daran, `dspec setup` auszuführen" },
    semantic: {
      ...EN.slash.semantic,
      description:
  ...
  setup: {
    ...
    apiKeyInvalid:
      "Schlüssel zu kurz — fügen Sie das vollständige Token ein (16+ Zeichen, keine Leerzeichen).",
    apiKeyChecking: "API-Schlüssel wird geprüft…",
    apiKeyRejected:
      "DeepSeek hat diesen API-Schlüssel abgelehnt. Fügen Sie einen gültigen Schlüssel ein oder drücken Sie Esc zum Abbrechen.",
    apiKeyCheckFailed:
  ...
  errors: {
    ...EN.errors,
    contextOverflow:
      "Context-Überlauf (DeepSeek 400): Sitzungsverlauf ist {requested}, über dem Prompt-Limit des Modells (V4: 1M Tokens; legacy chat/reasoner: 131k). Meist ist ein einzelnes Tool-Ergebnis zu groß geworden. DeepSeek-Spec begrenzt neue Tool-Ergebnisse auf 8k Tokens und heilt überdimensionierte Verläufe automatisch beim Sitzungsladen – ein Neustart behebt es oft. Falls es weiterhin überläuft, führe /new für einen frischen Start aus oder öffne /sessions und drücke [d], um diese Sitzung zu löschen.",
    contextOverflowTooMany: "Zu viele Tokens",
    auth401:
      "Authentifizierung fehlgeschlagen (DeepSeek 401): {inner}. Dein API-Schlüssel wird abgewiesen. Behebe mit `dspec setup` oder `export DEEPSEEK_API_KEY=sk-...`. Erhalte einen unter https://platform.deepseek.com/api_keys.",
    balance402:
    ...
    concurrency429:
      "DeepSeek-Gleichzeitigkeitslimit erreicht (429): {inner}. Das Konto hat zu viele gleichzeitige Anfragen (Grenze: 500 für v4-pro, 2500 für v4-flash, summiert über alle API-Schlüssel des Kontos). Meist läuft ein weiterer DeepSeek-Spec-Prozess mit demselben Schlüssel oder ein paralleler Subagent-Fan-out hat überzogen. Warte einige Sekunden und wiederhole, reduziere die Parallelität oder beantrage eine höhere Grenze unter https://platform.deepseek.com.",
    deepseek5xxHead:
      "DeepSeek-Dienst nicht verfügbar ({status}) — dies ist ein DeepSeek-seitiges Problem, nicht DeepSeek-Spec. Bereits 4× mit Backoff wiederholt.",
    ...
    upstream5xxHead:
      "Upstream-Dienst nicht verfügbar ({status}) bei {host} — der konfigurierte API-Endpunkt hat einen Serverfehler zurückgegeben, kein DeepSeek-Spec-Fehler. Bereits 4× mit Backoff wiederholt.",
  ...
  handlers: {
    ...
    basic: {
      ...
      helpSessionCustom: "  dspec chat --session <name>   eine andere benannte Sitzung verwenden",
      helpSessionNone: "  dspec chat --no-session       Persistenz für diesen Lauf deaktivieren",
      ...
      aboutHeader: "DeepSeek-Spec v{version} — ein Cache-First-DeepSeek-Coding-Agent",
    ...
    admin: {
      ...
      updateCurrent: "Aktuell: dspec {version}",
      updateLatestPending:
      ...
      updateRetryHint2:
        "oder führe `dspec update` in einem anderen Terminal aus, um es synchron zu erzwingen.",
      updateLatest: "Neueste:  dspec {version}",
      updateUpToDate: "Du bist auf dem neuesten Stand. Nichts zu tun.",
      updateNpxHint:
        "Du verwendest npx — der nächste `npx dspec ...`-Start lädt automatisch die neueste Version.",
      ...
      updateUpgradeCmd1: "  dspec update           (interaktiv, --dry-run wird unterstützt)",
    ...
    edits: {
      ...EN.handlers.edits,
      undoCodeOnly:
        "/undo ist nur innerhalb von `dspec code` verfügbar — der Chat-Modus wendet keine Edits an.",
      historyCodeOnly: "/history ist nur innerhalb von `dspec code` verfügbar.",
      showCodeOnly: "/show ist nur innerhalb von `dspec code` verfügbar.",
      applyCodeOnly:
        "/apply ist nur innerhalb von `dspec code` verfügbar (hier gibt es nichts anzuwenden).",
      discardCodeOnly: "/discard ist nur innerhalb von `dspec code` verfügbar.",
      planCodeOnly:
        "/plan ist nur innerhalb von `dspec code` verfügbar — der Chat-Modus blockiert keine Tool-Schreibzugriffe.",
      planOn:
      ...
      modeCodeOnly: "/mode ist nur innerhalb von `dspec code` verfügbar.",
      ...
      commitCodeOnly:
        "/commit ist nur innerhalb von `dspec code` verfügbar (benötigt ein Git-Repo als Wurzel).",
      commitUsage:
        'Verwendung: /commit "deine Commit-Nachricht"  — führt `git add -A && git commit -m "…"` in {root} aus',
      walkCodeOnly: "/walk ist nur innerhalb von `dspec code` verfügbar.",
      checkpointCodeOnly:
        "/checkpoint ist nur innerhalb von `dspec code` verfügbar — der Chat-Modus wendet keine Edits an.",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore ist nur innerhalb von `dspec code` verfügbar.",
      ...
      cwdCodeOnly: "/cwd ist nur innerhalb von `dspec code` verfügbar.",
      cwdUsage:
    ...
    model: {
      ...
      budgetNoCap:
        "Kein Sitzungsbudget festgelegt — DeepSeek-Spec wird weiterlaufen, bis du es stoppst. Setze eines mit: /budget <usd>   (z.B. /budget 5)",
      budgetStatus:
    ...
    permissions: {
      ...
      mutateCodeOnly:
        "/permissions add / remove / clear sind nur innerhalb von `dspec code` verfügbar — sie bearbeiten die projektbezogene Allowlist (`~/.dspec/config.json` projects[<root>].shellAllowed).",
      addUsage:
    ...
    plans: {
      ...EN.handlers.plans,
      noSession:
        "Keine Sitzung angehängt — `/plans` ist pro Sitzung. Führe `dspec code` in einem Projekt aus, um eine Sitzung zu erhalten.",
      activePlan:
      ...
      replayNoSession:
        "Keine Sitzung angehängt — `/replay` ist pro Sitzung. Führe `dspec code` in einem Projekt aus, um eine Sitzung zu erhalten.",
      replayNoArchives:
    ...
    jobs: {
      ...EN.handlers.jobs,
      codeOnly: "/jobs ist nur innerhalb von `dspec code` verfügbar.",
      killCodeOnly: "/kill ist nur innerhalb von `dspec code` verfügbar.",
      logsCodeOnly: "/logs ist nur innerhalb von `dspec code` verfügbar.",
      empty:
    ...
    mcp: {
      ...EN.handlers.mcp,
      noServers:
        'Keine MCP-Server angehängt. Führe `dspec setup` aus, um welche auszuwählen, oder starte mit --mcp "<spec>". `dspec mcp list` zeigt den Katalog. Hinweis: vom Modell aufgerufene Shell-Befehle werden pro Aufruf abgefragt (einmal erlauben / immer erlauben / ablehnen) — kein globales Allow-All-Flag.',
      ...
      catalogHint:
        "Vollständiger Katalog: `dspec mcp list` · tiefere Diagnose: `dspec mcp inspect <spec>`.",
      fallbackServers: "MCP-Server ({count}):",
      fallbackTools: "Tools im Register ({count}):",
      fallbackChange: "Um diesen Satz zu ändern, beende und führe `dspec setup` aus.",
      usageDisableEnable:
    ...
    skill: {
      ...EN.handlers.skill,
      listEmpty: "Keine Skills gefunden. DeepSeek-Spec liest Skills von:",
      listProjectScope:
        "  · <projekt>/.agents/skills/<name>/SKILL.md  (oder <name>.md)  — Projekt-Bereich",
      listGlobalScope: "  · ~/.agents/skills/<name>/SKILL.md  (oder <name>.md)  — globaler Bereich",
      listProjectOnly: "  (Projekt-Bereich ist nur in `dspec code` aktiv)",
  ...
  sessionPicker: {
    ...EN.sessionPicker,
    header: " ◈ DSPEC · Sitzung auswählen ",
  ...
  workspacePicker: {
    ...EN.workspacePicker,
    header: " ◈ DSPEC · Arbeitsbereich auswählen ",
  ...
  modelPicker: {
    ...EN.modelPicker,
    header: " ◈ DSPEC · Einrichtung auswählen ",
  ...
  startup: {
    ...EN.startup,
    codeRooted:
      '▸ dspec code: verwurzelt in {rootDir}, Sitzung "{session}" · {tools} native Tool{s}{semantic}',
    ephemeral: "(ephemer)",
  ...
  mcpHealth: {
    ...
    emptyHint:
      "ℹ keine MCP-Server konfiguriert — versuche: `dspec setup` zur erneuten Auswahl, oder `dspec mcp install filesystem` · Shell-Befehle werden pro Aufruf abgefragt (einmal erlauben / immer erlauben / ablehnen), kein globales Allow-All",
  },
  denyContextInput: {
    ...EN.denyContextInput,
    description:
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed:
      "✓ {name} installiert - starte `dspec code` neu zur Verbindung (Neuladen fehlgeschlagen: {message})",
    restartBridge: "✓ {name} installiert - starte `dspec code` neu zur Verbindung",
  ...
  mcpBrowser: {
    ...
    title: "◈ MCP-Browser",
    empty:
      "Keine MCP-Server angehängt. Führe `dspec setup` aus, um welche auszuwählen, oder starte mit --mcp.",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ führe `dspec setup` aus, um diesen Eintrag zu entfernen, oder behebe das zugrunde liegende Problem (fehlendes npm-Paket, Netzwerk usw.).",
    failedSetupConfigHint:
      "→ führe `dspec setup` aus, um fehlerhafte Einträge aus deiner gespeicherten Konfiguration zu entfernen.",
    abortedHint:
  ...
  checkpointPicker: {
    ...
    header: " ◈ DSPEC · Checkpoint auswählen ",
  ...
  mcpCli: {
    ...
    morePagesAvailable: "▸ mehr Seiten verfügbar — `dspec mcp list --pages <n>` oder --all",
    installHint: "Installieren:  dspec mcp install <name>",
    usageSearch: "Verwendung: dspec mcp search <abfrage>",
    usageInstall: "Verwendung: dspec mcp install <name>",
    ...
    moreLoaded: "… {count} weitere geladen — verwende `dspec mcp search <abfrage>` zum Filtern",
    moreMatches: "… {count} weitere Treffer",
    installed: "Installiert: {spec}",
    noServerFound:
      'Kein MCP-Server namens "{target}" gefunden nach {pages} Seite(n) der {source}-Registry.',
    noServerTryMore: "Versuche: dspec mcp install {target} --max-pages 100",
    noInstallMeta:
  ...
  scheduler: {
    ...
    historyStatusError: "✗",
    addUsage: "Verwendung: /schedule add <Name> --cron <Ausdruck> <Prompt>",
    addCreated: "Task '{name}' erstellt ({id}) — {description}",
    addCronInvalid:
      "Ungültiger Cron-Ausdruck: {reason}. Erwartet 5 Felder (Min Stunde Tag Monat WTag).",
    addCronDescription: "Zeitplan: {description}",
    listEmpty: "Keine geplanten Tasks. Verwenden Sie /schedule add zum Erstellen.",
    listHeader: "{name}  {cron}  {description}  {enabled}  {nextRun}  {lastRun}",
    listRow: "{name}  {cron}  {description}  {enabled}  {nextRun}  {lastRun}",
    listEnabled: "an",
    listDisabled: "aus",
    removeUsage: "Verwendung: /schedule remove <Name|id>",
    removeNotFound: "Kein Task mit Name oder id gefunden: {id}",
    removeSuccess: "Task '{name}' entfernt.",
    toggleUsage: "Verwendung: /schedule <enable|disable> <Name|id>",
    toggleNotFound: "Kein Task mit Name oder id gefunden: {id}",
    enableSuccess: "Task '{name}' aktiviert.",
    disableSuccess: "Task '{name}' deaktiviert.",
    helpText:
      "Verwendung: /schedule <Unterbefehl>\n\nUnterbefehle:\n  add <Name> --cron <Ausdruck> <Prompt>  Geplanten Task erstellen\n  list                                   Alle geplanten Tasks auflisten\n  remove <Name|id>                       Geplanten Task entfernen\n  enable <Name|id>                       Geplanten Task aktivieren\n  disable <Name|id>                      Geplanten Task deaktivieren\n  history <Name|id>                      Ausführungsverlauf anzeigen",
  },
};
