...
export const JA: TranslationSchema = {
  ...
  slashSuggestions: {
    ...
    groupDetailStrict: "strict-spec ワークフロー",
  ...
  builtinSkills: {
    ...
    strictInit:
      "現在のプロジェクトに .strict-spec-driven/ を初期化 — 設定、仕様、ロードマップディレクトリをスキャフォールドします。厳格ワークフローの前に最初に実行するのが最適です。",
    strictPropose:
      "新しい厳格仕様変更を提案 — .strict-spec-driven/changes/ に提案、設計、タスク、質問、差分仕様アーティファクトをスキャフォールドします。",
    strictApply:
      "現在の厳格変更でタスクを完了としてマークします。タスク状態マシンを進めます — 作業完了後にのみ呼び出してください。",
    strictVerify:
      "厳格変更が完了し一貫していることを検証 — タスク、差分仕様、仕様マッピングをチェックします。レビューとアーカイブの前に実行してください。",
    strictReview:
      "厳格変更のコード品質、仕様の整合性、テストカバレッジ、セキュリティをレビューします。安全な問題は自動修正し、曖昧な問題はエスカレーションします。",
    strictArchive:
      "検証済みでレビュー済みの厳格変更をアーカイブ — アクティブからアーカイブ済みに移動し、ロードマップマイルストーンの状態を調整します。",
    strictShip:
      "アーカイブ済みの厳格変更に対してフォーカスされたgitコミットを作成します。変更関連ファイルのみをステージングし、規約的なコミットメッセージを書き、プッシュはしません。",
    strictCancel:
      "進行中の厳格変更をキャンセル — キャンセル済みとしてマークし、オプションで適用済みのコード変更を元に戻します。",
    strictBrainstorm:
      "大まかなアイデアから厳格変更をブレインストームして形成 — 対話的な洗練の後、準備ができたら提案をスキャフォールドします。",
    strictModify:
      "進行中の厳格変更を修正 — スコープ、要件、タスク、設計決定を更新します。スコープを黙って拡大することはありません。",
    strictRoadmapRecommend:
      "次のロードマップ支援された厳格変更を推奨し、自動的にスキャフォールドします。ロードマップの依存関係と完了状態からスクリプトによる優先度を使用します。",
    strictRoadmapPlan:
      "厳格ロードマップを作成または再構築 — マイルストーン、計画変更、依存関係を追加します。roadmap-sync を実行して状態を調整します。",
    strictAuto:
      "厳格ライフサイクルをエンドツーエンドで実行 — 提案、実装、検証、レビュー、アーカイブ。未解決の質問が進行をブロックする場合を除き自動的に進行します。",
  },
  shortcutsHelp: {
    title: "ショートカット",
    groupInput: "入力",
  ...
  scheduler: {
    ...
    historyStatusError: "✗",
    addUsage: "使い方：/schedule add <名前> --cron <式> <プロンプト>",
    addCreated: "タスク '{name}' を作成しました ({id}) — {description}",
    addCronInvalid: "無効な cron 式：{reason}。5 つのフィールドが必要です（分 時 日 月 曜日）。",
    addCronDescription: "スケジュール：{description}",
    listEmpty: "スケジュールされたタスクはありません。/schedule add で作成してください。",
    listHeader: "{name}  {cron}  {description}  {enabled}  {nextRun}  {lastRun}",
    listRow: "{name}  {cron}  {description}  {enabled}  {nextRun}  {lastRun}",
    listEnabled: "オン",
    listDisabled: "オフ",
    removeUsage: "使い方：/schedule remove <名前|id>",
    removeNotFound: "名前または id {id} のタスクが見つかりません",
    removeSuccess: "タスク '{name}' を削除しました。",
    toggleUsage: "使い方：/schedule <enable|disable> <名前|id>",
    toggleNotFound: "名前または id {id} のタスクが見つかりません",
    enableSuccess: "タスク '{name}' を有効にしました。",
    disableSuccess: "タスク '{name}' を無効にしました。",
    helpText:
      "使い方：/schedule <サブコマンド>\n\nサブコマンド：\n  add <名前> --cron <式> <プロンプト>  タスクを作成\n  list                                 タスク一覧\n  remove <名前|id>                     タスクを削除\n  enable <名前|id>                     タスクを有効化\n  disable <名前|id>                    タスクを無効化\n  history <名前|id>                    実行履歴を表示",
  },
};
