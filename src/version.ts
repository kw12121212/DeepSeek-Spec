...
export function detectInstallSource(bin?: string): InstallSource {
  ...
  if (/\/node_modules\/deepseek-spec(\b|\/)/.test(norm)) return "npm";
  return "unknown";
}
...
export function detectNpmInstallPrefix(bin?: string): string | null {
  ...
  const posix = norm.match(/^(.+?)\/lib\/node_modules\/deepseek-spec(?:\/|$)/i);
  if (posix) return posix[1] ?? null;
  const win = norm.match(/^(.+?)\/node_modules\/deepseek-spec(?:\/|$)/i);
