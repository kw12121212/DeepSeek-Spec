# DeepSeek-Spec — working knowledge
...
- **This IS DeepSeek-Spec** — edits to `src/loop.ts`, `src/repair/`, `src/tools/`, `src/mcp/` affect every session. Test before publishing.
...
- **`.dspec/semantic/`** is auto-generated vector index. Never hand-edit.
- **`sessions/` and `.dspec/sessions/`** are user-private, git-ignored (per `.gitignore`).
