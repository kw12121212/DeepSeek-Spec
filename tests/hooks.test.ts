...
function writeSettings(dir: string, json: unknown): string {
  const path = join(dir, ".dspec", "settings.json");
  mkdirSync(join(dir, ".dspec"), { recursive: true });
...
describe("loadHooks", () => {
  ...
  it("tolerates malformed JSON without throwing", () => {
    ...
    writeFileSync(join(home, ".dspec", "settings.json"), "{ not valid json", "utf8");
  ...
  it("paths reported by *SettingsPath helpers are absolute", () => {
    expect(globalSettingsPath(home)).toBe(join(home, ".dspec", "settings.json"));
    expect(projectSettingsPath(project)).toBe(join(project, ".dspec", "settings.json"));
  });
});
