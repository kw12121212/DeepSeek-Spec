...
describe("registerWebTools", () => {
  const isolatedConfigPath = "/tmp/dspec-web-tools-test-config-does-not-exist.json";
  ...
  it("web_search can dispatch through Ollama cloud", async () => {
    const configPath = "/tmp/dspec-web-tools-ollama-search-test.json";
    ...
    globalThis.fetch = vi.fn(async (_url, init) => {
      ...
      return new Response(
        JSON.stringify({
          results: [
            { title: "DeepSeek-Spec", url: "https://example.com/r", content: "Result body" },
          ],
        }),
    ...
    try {
      ...
      const out = await registry.dispatch(
        "web_search",
        JSON.stringify({ query: "dspec", topK: 2 }),
      );
  ...
  it("web_fetch can dispatch through Ollama cloud when selected", async () => {
    const configPath = "/tmp/dspec-web-tools-ollama-fetch-test.json";
    writeConfig({ webSearchEngine: "ollama", ollamaApiKey: "ollama-test-key" }, configPath);
    const originalFetch = globalThis.fetch;
    globalThis.fetch = vi.fn(async (_url, init) => {
