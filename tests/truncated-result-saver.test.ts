...
describe("saveTruncatedResult", () => {
  ...
  it("writes content to .dspec/truncated-results/", () => {
    ...
    expect(path).toMatch(/^\.dspec\/truncated-results\/\d+-[a-f0-9]+-web_search\.txt$/);
  ...
  it("creates the directory if missing", () => {
    ...
    expect(existsSync(join(rootDir, ".dspec", "truncated-results"))).toBe(true);
  ...
  it("falls back to ~/.dspec when rootDir is the filesystem root", () => {
    ...
    try {
      ...
      expect(path).toMatch(/\.dspec\/truncated-results\//);
      expect(path.startsWith("/.dspec")).toBe(false);
  ...
  it("falls back to ~/.dspec when rootDir is empty", () => {
...
describe("cleanupOldResults", () => {
  ...
  it("removes files older than maxAgeMs", () => {
    const dir = join(rootDir, ".dspec", "truncated-results");
