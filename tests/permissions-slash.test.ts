...
describe("/permissions slash handler", () => {
  ...
  it("bare /permissions lists project entries with 1-based indices", () => {
    addProjectShellAllowed(projectRoot, "npm run build", join(dir, ".dspec", "config.json"));
    addProjectShellAllowed(projectRoot, "deploy.sh", join(dir, ".dspec", "config.json"));
    const result = handleSlash("permissions", [], makeLoop(), {
      codeRoot: projectRoot,
      editMode: "review",
    });
  ...
  it("/permissions add persists a new prefix", () => {
    ...
    expect(loadProjectShellAllowed(projectRoot, join(dir, ".dspec", "config.json"))).toContain(
      "npm run build",
    );
  });
  ...
  it("/permissions add notes when the target is already in the builtin list", () => {
    ...
    expect(loadProjectShellAllowed(projectRoot, join(dir, ".dspec", "config.json"))).toEqual([]);
  });

  it("/permissions remove drops by exact prefix", () => {
    const cfgFile = join(dir, ".dspec", "config.json");
