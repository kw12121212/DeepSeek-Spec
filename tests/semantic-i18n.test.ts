...
describe("semantic i18n", () => {
  ...
  describe("detectLocale", () => {
    it("returns 'zh' when DSPEC_LANG=zh", () => {
      process.env.DSPEC_LANG = "zh";
      expect(detectLocale()).toBe("zh");
    });

    it("returns 'en' when DSPEC_LANG=en (overrides system locale)", () => {
      process.env.DSPEC_LANG = "en";
      process.env.LANG = "zh_CN.UTF-8";
      expect(detectLocale()).toBe("en");
    });

    it("returns 'zh' for LANG=zh_CN.UTF-8", () => {
      process.env.DSPEC_LANG = undefined;
      process.env.LANG = "zh_CN.UTF-8";
      expect(detectLocale()).toBe("zh");
    });
    ...
    it("returns 'en' when LANG points elsewhere and no override", () => {
      ...
      // when DSPEC_LANG is absent. We don't pin Intl here because
