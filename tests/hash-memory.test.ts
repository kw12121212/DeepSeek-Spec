...
describe("appendProjectMemory", () => {
  ...
  it("creates DSPEC.md with a header and the first bullet when absent", () => {
    const path = join(dir, "DSPEC.md");
  ...
  it("appends to an existing DSPEC.md without disturbing earlier content", () => {
    const path = join(dir, "DSPEC.md");
  ...
  it("appends multiple bullets in order across calls", () => {
    ...
    const content = readFileSync(join(dir, "DSPEC.md"), "utf8");
  ...
  it("respects nested rootDir paths (creates DSPEC.md in the given dir, not cwd)", () => {
    const nested = join(dir, "subproject");
    mkdirSync(nested);
    const result = appendProjectMemory(nested, "scoped note");
    expect(result.path).toBe(join(nested, "DSPEC.md"));
    expect(existsSync(join(dir, "DSPEC.md"))).toBe(false);
  });
});
...
describe("appendGlobalMemory", () => {
  ...
  it("creates ~/.dspec/DSPEC.md (with parent dir) when missing", () => {
  ...
  it("uses os.homedir() when no override is passed (smoke check)", () => {
    ...
    expect(path).toMatch(/[/\\]\.dspec[/\\]DSPEC\.md$/);
  });
