...
async function call(
  ...
  if (method !== "GET") headers["X-Dspec-Token"] = TOKEN;
...
describe("dashboard /sessions: new / switch / delete (attached)", () => {
  ...
  beforeEach(async () => {
    dir = mkdtempSync(join(tmpdir(), "dspec-sess-ops-"));
    // Re-home so listSessions reads from our temp dir, not the user's real ~/.dspec.
  ...
  it("GET /api/sessions filters out other-workspace sessions when getCurrentCwd is wired", async () => {
    await handle?.close();
    const otherWorkspace = mkdtempSync(join(tmpdir(), "dspec-other-ws-"));
    try {
