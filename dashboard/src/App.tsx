  ...
  switch (action.t) {
    ...
    case "rpc_exit":
      return {
        ...
        messages: [
          ...state.messages,
          { kind: "error", message: `dspec exited (code ${action.code ?? "?"})` },
        ],
      };
    case "incoming":
...
function TabRuntime({
  ...
  const slashCommands: SlashCmd[] = [
    ...
    },
    {
      ...
      run: () => {
        void openUrl("https://github.com/esengine/DeepSeek-Spec/issues/new/choose").catch(
          () => undefined,
        );
      },
    },
...
export function App() {
  ...
  const [sideCollapsed, setSideCollapsed] = useState(
    () => localStorage.getItem("dspec.sideCollapsed") === "1",
  );
  const [ctxCollapsed, setCtxCollapsed] = useState(
    () => localStorage.getItem("dspec.ctxCollapsed") === "1",
  );
  ...
  useEffect(() => {
    localStorage.setItem("dspec.sideCollapsed", sideCollapsed ? "1" : "0");
  }, [sideCollapsed]);

  useEffect(() => {
    localStorage.setItem("dspec.ctxCollapsed", ctxCollapsed ? "1" : "0");
  }, [ctxCollapsed]);
